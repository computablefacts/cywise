...
class SendAuditReportListener extends AbstractListener
{
    ...
    private function buildSummary(User $user, Collection $assets): string
    {
        ...
        $newAssets = $nbNewAssets === 0 ?
            '' :
            "<li>J'ai mis sous surveillance <b>{$nbNewAssets}</b> nouveaux actifs durant ces dernières 24h.</li>";

        $leaks = $nbLeaks === 0 ?
            '' :
            "<li>J'ai trouvé <b>{$nbLeaks}</b> identifiants fuités ou compromis.</li>";

        $perimeter = $nbDns + $nbIpAddresses === 0 ?
            '' :
            "<li>J'ai analysé <b>{$nbDns}</b> domaines et <b>{$nbIpAddresses}</b> serveurs.</li>";

        $high = $nbHigh === 0 ?
            '' :
            "<li><b>{$nbHigh}</b> vulnérabilités critiques <b>doivent</b> être corrigées.</li>";

        $medium = $nbMedium === 0 ?
            '' :
            "<li><b>{$nbMedium}</b> vulnérabilités de criticité moyenne <b>devraient</b> être corrigées.</li>";

        $low = $nbLow === 0 ?
            '' :
            "<li><b>{$nbLow}</b> vulnérabilités de criticité basse ne posent pas un risque de sécurité immédiat.</li>";
