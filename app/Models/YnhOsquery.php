...
class YnhOsquery extends Model
{
    ...
    public function message(): string
    {
        ...
        } else if ($this->name === 'nmap_process' ||
            $this->name === 'cywise_nmap_usage') {
            if ($this->isAdded()) {
                $msg = "Un scan nmap a été lancé : {$this->columns['cmdline']}";
            }
            if ($this->isRemoved()) {
                $msg = "Un scan nmap a été arrêté : {$this->columns['cmdline']}";
            }
        } else if ($this->name === 'cywise_nuclei_usage') {
            if ($this->isAdded()) {
                $msg = "Un scan nuclei a été lancé : {$this->columns['cmdline']}";
            }
            if ($this->isRemoved()) {
                $msg = "Un scan nuclei a été arrêté : {$this->columns['cmdline']}";
            }
        } else if ($this->name === 'hidden_files') {
            if ($this->isAdded()) {
                $msg = "Le fichier caché {$this->columns['path']} a été créé.";
            }
            if ($this->isRemoved()) {
                $msg = "Le fichier caché {$this->columns['path']} a été supprimé.";
            }
