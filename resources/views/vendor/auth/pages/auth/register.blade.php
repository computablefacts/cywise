...
/**
 * Adapt from vendor/devdojo/auth/resources/views/pages/auth/register.blade.php
 * Compare with original to view differences
 *
 * Use this view for invitation link too
 */
...
new class extends Component {
    ...
    public function mount()
    {
        ...
        if (request()->has('invitation')) {
            ...
            $invitation = Invitation::query()->where('token', $invitationToken)->first();

            if (!$invitation) {
                session()->flash('error', config('devdojo.auth.language.register.invitation_not_valid', 'Invalid invitation.'));
                return redirect()->route('auth.login');
            }

            if ($invitation->isExpired()) {
                session()->flash('error', config('devdojo.auth.language.register.invitation_expired', 'Expired invitation.'));
                return redirect()->route('auth.login');
            }

            if ($invitation->alreadyUsed()) {
                session()->flash('error', config('devdojo.auth.language.register.invitation_already_used', 'This invitation has already be used.'));
                return redirect()->route('auth.login');
            }
    ...
    public function register()
    {
        ...
        if ($this->hasInvitation) {
            $invitation = Invitation::query()->where('token', $this->invitationToken)->first();
