            ...
            <div class="flex lg:flex-row flex-col @guest pb-20 pt-10 @endguest space-x-5 h-full">
                <x-card class="flex flex-col justify-center items-center p-10 w-full lg:w-1/3">
                        ...
                        @if (auth()->check() && auth()->user()->isAdmin())
                            <a href="{{ route('impersonate', $this->user->id) }}" wire:navigate class="px-3 py-1 my-2 text-xs font-medium text-white rounded-sm text-zinc-600 bg-zinc-200">Impersonate</a>
                        @endif
