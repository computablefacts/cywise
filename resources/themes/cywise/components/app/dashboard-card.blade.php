<a href="{{ $href ?? '' }}" @if($target ?? false) target="_blank" @else wire:navigate @endif class="flex overflow-hidden relative p-5 w-full bg-white dark:bg-zinc-800 rounded-lg border duration-300 ease-out group border-slate-200 dark:border-zinc-700 hover:scale-[1.01]">
    <span class="flex relative flex-col justify-center items-start pr-0 pb-1 space-y-3 h-full">
        ...
        <span class="inline-flex relative justify-start items-center -mt-1 mb-2 w-auto text-xs tracking-tight leading-none text-slate-600 dark:text-slate-300">
            <span class="inline-block shrink-0 mr-0">{{ $linkText ?? '' }}</span>
