<div x-data="{ {{ $id }}: {{ $open ?? false }} }"
    ...
    <div
        @click="{{ $id }}=!{{ $id }}"
        class="@if($active){{ 'text-zinc-900 bg-white border-zinc-200 shadow-xs font-medium dark:bg-zinc-700/60 dark:text-zinc-100' }}@endif ease-linear duration-50 transition-colors flex rounded-lg w-full h-auto px-2.5 py-2 cursor-pointer text-sm border justify-start items-center overflow-hidden group-hover:autoflow-auto items"
        :class="{ 'text-zinc-900 bg-white border-zinc-200 dark:border-zinc-700 shadow-xs font-medium dark:bg-zinc-700/60 dark:text-zinc-100' : {{ $id }} == true, 'hover:bg-zinc-100 dark:hover:bg-zinc-700/60 hover:text-zinc-900 border-transparent dark:hover:text-zinc-100' : ({{ $id }} == false && {{ !$active }}) }"
    >
        <div class="flex relative items-center w-full h-auto">
            <x-dynamic-component :component="$icon" class="shrink-0 mr-2 w-5 h-5" />
