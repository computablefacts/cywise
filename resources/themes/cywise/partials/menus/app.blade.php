<div x-data="{ open: false }" class="flex h-full md:flex-1">
    <div class="hidden flex-1 space-x-8 h-full font-semibold md:flex">
        <a href="{{ route('wave.dashboard') }}" wire:navigate class="inline-flex items-center px-1 pt-1 text-sm leading-5 transition duration-150 ease-in-out focus:outline-hidden border-b-2 border-transparent @if(Request::is('dashboard')){{ 'text-zinc-900' }}@else{{ 'text-zinc-800 hover:text-zinc-900' }}@endif">Dashboard</a>
    ...
    <div class="flex sm:ml-6 sm:items-center">
        ...
        <div @click.away="open = false" class="flex relative items-center ml-3 h-full" x-data="{ open: false }">
            <div>
                <button @click="open = !open" class="flex text-sm rounded-full border-2 border-transparent transition duration-150 ease-in-out focus:outline-hidden focus:border-zinc-300" id="user-menu" aria-label="User menu" aria-haspopup="true" x-bind:aria-expanded="open" aria-expanded="true">
            ...
            <div
                ...
                <div class="bg-white rounded-xl border shadow-md border-zinc-100" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                    ...
                    @impersonating
                            <a href="{{ route('impersonate.leave') }}" wire:navigate class="block px-4 py-2 text-sm leading-5 text-blue-900 bg-blue-50 border-t text-zinc-700 border-zinc-100 hover:bg-blue-100 focus:outline-hidden focus:bg-blue-200">Leave impersonation</a>
                    @endImpersonating
                    <div class="border-t border-zinc-100"></div>
                    <div class="py-1">

                        <div class="block px-4 py-1">
                            <span class="inline-block px-2 my-1 -ml-1 text-xs font-medium leading-5 rounded-sm text-zinc-600 bg-zinc-200">{{ auth()->user()->roles->first()->name }}</span>
                        </div>
                        @trial
                            <a href="{{ route('wave.settings', 'plans') }}" wire:navigate class="block px-4 py-2 text-sm leading-5 text-zinc-700 hover:bg-zinc-100 hover:text-zinc-900 focus:outline-hidden focus:bg-zinc-100 focus:text-zinc-900">Upgrade My Account</a>
                        @endtrial
                        @if( !auth()->guest() && auth()->user()->isAdmin() )
                            <a href="/admin" wire:navigate class="block px-4 py-2 text-sm leading-5 text-zinc-700 hover:bg-zinc-100 hover:text-zinc-900 focus:outline-hidden focus:bg-zinc-100 focus:text-zinc-900"><i class="fa fa-bolt"></i> Admin</a>
                        @endif
                        <a href="{{ route('wave.profile', auth()->user()->username) }}" wire:navigate class="block px-4 py-2 text-sm leading-5 text-zinc-700 hover:bg-zinc-100 hover:text-zinc-900 focus:outline-hidden focus:bg-zinc-100 focus:text-zinc-900">My Profile</a>
                        <a href="{{ route('wave.settings') }}" wire:navigate class="block px-4 py-2 text-sm leading-5 text-zinc-700 hover:bg-zinc-100 hover:text-zinc-900 focus:outline-hidden focus:bg-zinc-100 focus:text-zinc-900">Settings</a>

                    </div>
                    <div class="border-t border-zinc-100"></div>
                    <div class="py-1">
                        <a href="{{ route('logout') }}" wire:navigate class="block px-4 py-2 w-full text-sm leading-5 text-left text-zinc-700 hover:bg-zinc-100 hover:text-zinc-900 focus:outline-hidden focus:bg-zinc-100 focus:text-zinc-900" role="menuitem">
                            Sign out
                        </a>
                    </div>
                </div>
