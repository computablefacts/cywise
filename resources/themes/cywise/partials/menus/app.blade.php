<div x-data="{ open: false }" class="flex h-full md:flex-1">
    <div class="hidden flex-1 space-x-8 h-full font-semibold md:flex">
        <a href="{{ route('wave.dashboard') }}" class="inline-flex items-center px-1 pt-1 text-sm leading-5 transition duration-150 ease-in-out focus:outline-hidden border-b-2 border-transparent @if(Request::is('dashboard')){{ 'text-zinc-900' }}@else{{ 'text-zinc-800 hover:text-zinc-900' }}@endif">Dashboard</a>
        ...
        <div @click.away="open = false" class="flex relative items-center ml-3 h-full" x-data="{ open: false }">
            ...
            <div
                ...
                <div class="bg-white rounded-xl border shadow-md border-zinc-100" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                    ...
                    @impersonating
                            <a href="{{ route('impersonate.leave') }}" class="block px-4 py-2 text-sm leading-5 text-blue-900 bg-blue-50 border-t text-zinc-700 border-zinc-100 hover:bg-blue-100 focus:outline-hidden focus:bg-blue-200">Leave impersonation</a>
                    @endImpersonating
                    ...
                    <div class="py-1">
                        ...
                        @trial
                            <a href="{{ route('wave.settings', 'plans') }}" class="block px-4 py-2 text-sm leading-5 text-zinc-700 hover:bg-zinc-100 hover:text-zinc-900 focus:outline-hidden focus:bg-zinc-100 focus:text-zinc-900">Upgrade My Account</a>
                        @endtrial
                        @if( !auth()->guest() && auth()->user()->isAdmin() )
                            <a href="/admin" class="block px-4 py-2 text-sm leading-5 text-zinc-700 hover:bg-zinc-100 hover:text-zinc-900 focus:outline-hidden focus:bg-zinc-100 focus:text-zinc-900"><i class="fa fa-bolt"></i> Admin</a>
                        @endif
                        <a href="{{ route('wave.profile', auth()->user()->username) }}" class="block px-4 py-2 text-sm leading-5 text-zinc-700 hover:bg-zinc-100 hover:text-zinc-900 focus:outline-hidden focus:bg-zinc-100 focus:text-zinc-900">My Profile</a>
                        <a href="{{ route('wave.settings') }}" class="block px-4 py-2 text-sm leading-5 text-zinc-700 hover:bg-zinc-100 hover:text-zinc-900 focus:outline-hidden focus:bg-zinc-100 focus:text-zinc-900">Settings</a>

                    </div>
                    <div class="border-t border-zinc-100"></div>
                    <div class="py-1">
                        <a href="{{ route('logout') }}" class="block px-4 py-2 w-full text-sm leading-5 text-left text-zinc-700 hover:bg-zinc-100 hover:text-zinc-900 focus:outline-hidden focus:bg-zinc-100 focus:text-zinc-900" role="menuitem">
                            Sign out
                        </a>
                    </div>
                </div>
